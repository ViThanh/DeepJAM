...
# Plot results ------------------------------------------------------------
plot.DeepJAM <- \(object, data, K, mu = NULL, s, seed = NULL,...){
  ...
  if(length(dims) == 2){
    ...
    for(i in c("observed", "aligned")){
      dt <- if(i == "observed") t(data) else t(as.array(Warp(tf$cast(data, dtype = total_warps$dtype), total_warps)))
      matplot(seq(0, 1, length.out = L) * K, dt[,idx], col = 0, type = "l", xlab = "", ylab = "", xaxt = "n", yaxt = "n", bty = "n", main = if(i == "observed") "Observed data" else "Aligned data")
    ...
    layout(matrix(1:(2 * n), nrow = n))
    par(mar = c(2, 0, 0, 0))
    for(i in c("observed", "aligned")){
      for(j in seq_len(n)){
        dt <- if(i == "observed") t(data[,,j]) else t(as.array(Warp(tf$cast(data[,,j], dtype = total_warps$dtype), total_warps)))
        ...
        axis(side = 1, at = c(0, seq_len(K)), labels = c(0, expression(tau), sapply(seq_len(K - 1) + 1, \(x) as.expression(bquote(.(x) * tau)))))
        if(j == 1) title(if(i == "observed") "Observed data" else "Aligned data", line = -1)
      }
    }
    layout(1)
  }
